...
class Behaviour {
  public:
    Behaviour()                                            = default;
    virtual void frame(Sim& sim, const sf::Window& window) = 0;
...
class SoftBody : public Behaviour {
  ...
  public:
    ...
    void frame(Sim& sim, const sf::Window& window) override {
        ...
        // if the current closest is close enough delete it. then search for all others in range and delete them
        if (sf::Mouse::isButtonPressed(sf::Mouse::Right) && closestDist < sliceRange) {
            sim.removePoint(*closestPoint);
            closestPoint.reset();
            if(sim.points.size() == 0) return;
            auto pointsInRange = sim.findPointsInRange(mousePos, sliceRange); // this is in reverse order so I can delete without errors
